...
class Item {
  private _worryLevel: bigint;

  public get worryLevel (): bigint {
    return this._worryLevel;
  }

  private set worryLevel (value: bigint) {
    this._worryLevel = value;
  }

  private _lcm: bigint = BigInt(1);
  public get lcm (): bigint {
    return this._lcm;
  }

  public set lcm (value: bigint) {
    this._lcm = value;
  }

  constructor (worryLevel: number) {
    this._worryLevel = BigInt(worryLevel);
  }

  public addToWorry (amount: bigint): void {
    this.worryLevel += amount;
  }

  public multiplyToWorry (amount: bigint): void {
    this.worryLevel *= amount;
  }

  public postInspection (): void {
    // this.worryLevel = Math.floor(this.worryLevel / 3);
    console.log(this.worryLevel, this.lcm, this.worryLevel % this.lcm);
    this.worryLevel %= this.lcm;
  }
...
class Monkey {
  ...
  private _testDivisibleBy: bigint;
  public get testDivisibleBy (): bigint {
    return this._testDivisibleBy;
  }

  public set testDivisibleBy (value: bigint) {
    this._testDivisibleBy = value;
  }
  ...
  constructor (monkeyNumber: number, items: Item[], testDivisibleBy: number, operation: (item: Item) => void) {
    ...
    this._items = items;
    this._testDivisibleBy = BigInt(testDivisibleBy);
  ...
  public processItems (): void {
    this.items.forEach(item => {
      ...
      this.operation(item);
      item.postInspection();
      if (item.worryLevel % this.testDivisibleBy === BigInt(0)) {
...
void (async () => {
  ...
  monkeyTexts.forEach((text) => {
    ...
    monkeyArr[monkeyNumber] = new Monkey(monkeyNumber, items.split(', ').map((item) => new Item(parseInt(item, 10))), parseInt(divisibleBy, 10), (item) => {
      let operatorValue: bigint;
      if (operatorValueText === 'old') {
        operatorValue = item.worryLevel;
      } else {
        operatorValue = BigInt(parseInt(operatorValueText, 10));
      }
  ...
  const lcm = trimmedMonkeyArr.reduce<bigint>((value, monkey) => value * monkey.testDivisibleBy, BigInt(1));
  trimmedMonkeyArr.forEach(monkey => { monkey.items.forEach(item => { item.lcm = lcm; }); });
